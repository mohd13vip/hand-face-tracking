...
"""
=============================================================
  ENHANCED COMPUTER VISION TRACKING SYSTEM  -  v4.0 (Mac/MPS)
=============================================================
  MediaPipe Tasks API  (mediapipe 0.10+)
  YOLOv8-Pose for MPS-accelerated arm tracking (Apple Silicon)

  FEATURES
  --------
  [1]  YOLO-POSE person detection + arm keypoints  (MPS/CPU)
       Replaces both YOLO person-detect AND MediaPipe pose.
       One model, one MPS call, better accuracy.
  ...
  python person_hand_finger_mac.py \
      --index 0 --backend any --model yolov8s-pose.pt
=============================================================
"""
...
if torch.backends.mps.is_available():
    DEVICE = "mps"
    print("=" * 50)
    print("  MPS ACTIVE: Apple Silicon GPU")
    print("=" * 50)
else:
    DEVICE = "cpu"
...
# ═══════════════════════════════════════════════════════════════
#  POSE SMOOTHER - EMA for YOLO-Pose MPS keypoints
#
...
def main():
    ...
    ap.add_argument("--backend",    type=str,   default="any",
    ...
    yolo.model.eval()
    if DEVICE == 'mps':
        print("YOLO running on: Apple Silicon MPS")
    tracker=Tracker()
    ...
    # No separate pose model needed — YOLO-Pose handles it on MPS!
    print("  YOLO-Pose: person detection + arm keypoints (MPS)")
    ...
    while True:
        ...
        # 1. YOLO-POSE — person detection + arm keypoints in ONE MPS call
        ...
        if face_res.face_landmarks:
            for fi,flms in enumerate(face_res.face_landmarks):
                ...
                if bs_list and len(bs_list) > 10:
                    ...
                    neutral_raw = [("Neutral",1.0,(180,180,180))] + \
                                  [(lbl,0.0,EMOTION_COLORS[lbl]) for lbl in EMOTION_ORDER if lbl!="Neutral"]
